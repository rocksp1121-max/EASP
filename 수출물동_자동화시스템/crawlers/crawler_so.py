...
"""
crawler_so.py
Sales Order Progress 데이터 자동 추출
포털 화면: Display Sales Order Progress

실행:
    python crawler_so.py
    python crawler_so.py --headless

TODO: 사용자로부터 화면명/경로 확인 후 아래 SCREEN_NAME, 쿼리 조건 업데이트 필요
"""
...
SCREEN_NAME = "Display Sales Order Progress"   # ← 포털 검색어 (확인 필요)
OUTPUT_NAME = "Display Sales Order Progress.xlsx"  # import_raw.py 기대 파일명


def navigate_and_query(page):
    """포털 검색 → SO 화면 진입 → 조건 입력 → 조회"""
    ...
    # ── 조회 조건 입력 ──────────────────────────────────────
    # TODO: 사용자로부터 필터 필드(Plant, Sales Org 등) 확인 후 아래 코드 추가
    # 예시:
    # frame.locator("input[aria-label='Plant']").fill("DFZ")
    # ────────────────────────────────────────────────────────
